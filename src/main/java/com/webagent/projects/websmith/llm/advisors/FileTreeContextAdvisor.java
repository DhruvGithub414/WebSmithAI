package com.webagent.projects.websmith.llm.advisors;
...
public class FileTreeContextAdvisor implements StreamAdvisor {
    ...
    public Flux<ChatClientResponse> adviseStream(ChatClientRequest request, StreamAdvisorChain streamAdvisorChain) {
        ...
        ChatClientRequest augmented = augmentRequestWithFileTree(request, projectId);

        Flux<ChatClientResponse> chatClientResponseFlux = streamAdvisorChain.nextStream(augmented);
    ...
    private ChatClientRequest augmentRequestWithFileTree(ChatClientRequest request, Long projectId){
        ...
        return request.mutate()
                .prompt(new Prompt(allMessages, request.prompt().getOptions())).build();
